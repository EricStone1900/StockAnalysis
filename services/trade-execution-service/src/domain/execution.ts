...
export interface ApprovedExecutionCommand { readonly rebalanceBatchId: string; readonly decisionId: string; readonly proposalId?: string; readonly portfolioId?: string; readonly proposalVersion: number; readonly approvalId: string; readonly riskEvaluationId: string; readonly budgetReservationId: string; /** 真实运行必填；保留可选以兼容历史Fixture，权威授权端口拒绝缺失值。 */ readonly resourceReservationId?: string; readonly targetPortfolioVersion: number; readonly validUntil: string; readonly legs: readonly { legId: string; securityId: string; side: 'BUY' | 'SELL'; quantity: string }[]; readonly contentHash: string; readonly idempotencyKey: string; /** 供真实 Temporal Event Starter 使用的已验证工作流输入，不参与执行内容Hash。 */ readonly workflowRequest?: Record<string, unknown>; }
...
export interface RebalanceBatch { readonly rebalanceBatchId: string; readonly decisionId: string; readonly proposalId?: string; readonly portfolioId?: string; readonly proposalVersion: number; readonly approvalId: string; readonly riskEvaluationId: string; readonly budgetReservationId: string; readonly resourceReservationId?: string; readonly targetPortfolioVersion: number; readonly validUntil: string; readonly contentHash: string; readonly intents: readonly OrderIntent[]; readonly workflowRequest?: Record<string, unknown>; }
...
export function executionDigest(command: ApprovedExecutionCommand): string {
  const canonical = { rebalanceBatchId: command.rebalanceBatchId, decisionId: command.decisionId, ...(command.proposalId ? { proposalId: command.proposalId } : {}), ...(command.portfolioId ? { portfolioId: command.portfolioId } : {}), proposalVersion: command.proposalVersion, approvalId: command.approvalId, riskEvaluationId: command.riskEvaluationId, budgetReservationId: command.budgetReservationId, ...(command.resourceReservationId ? { resourceReservationId: command.resourceReservationId } : {}), targetPortfolioVersion: command.targetPortfolioVersion, validUntil: command.validUntil, legs: command.legs };
...
export class ExecutionAggregate {
  ...
  getBatch(batchId: string): RebalanceBatch | undefined { return this.batches.get(batchId); }
  createApprovedBatch(command: ApprovedExecutionCommand): RebalanceBatch {
    ...
    const batch = { ...canonical, contentHash: command.contentHash, intents, ...(command.workflowRequest ? { workflowRequest: command.workflowRequest } : {}) }; this.batches.set(command.rebalanceBatchId, batch); this.idempotency.set(command.idempotencyKey, batch); return batch;
  }
