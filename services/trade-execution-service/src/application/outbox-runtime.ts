...
export class ExecutionNatsPublisher { public constructor(private readonly connection: ExecutionJetStreamConnection) {} public async publish(event: ExecutionOutboxRecord): Promise<void> { await this.connection.jetstream().publish(event.subject, new TextEncoder().encode(JSON.stringify(event.payload))); } }
