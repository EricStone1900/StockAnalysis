...
async function run(): Promise<void> {
  ...
  const starter = new RebalanceExecutionEventStarter(nats, await nats.jetstreamManager(), new Client({ connection: temporal }), process.env.WORKFLOW_TASK_QUEUE ?? 'stock-workflows-v1');
  await starter.start();
