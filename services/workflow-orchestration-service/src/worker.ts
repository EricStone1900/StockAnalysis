...
import { assertDemoWorkerMode } from './worker-mode.js';

async function run(): Promise<void> {
  assertDemoWorkerMode(process.env);
  ...
  const worker = await Worker.create({ connection, taskQueue: process.env.WORKFLOW_TASK_QUEUE ?? WORKFLOW_TASK_QUEUE, workflowsPath: new URL('./workflows/index.ts', import.meta.url).pathname, activities });
  await worker.run();
}
void run();
