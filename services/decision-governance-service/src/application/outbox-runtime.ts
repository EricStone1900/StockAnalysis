...
export class GovernanceNatsPublisher { public constructor(private readonly connection: GovernanceJetStreamConnection) {} public async publish(event: GovernanceOutboxRecord): Promise<void> { await this.connection.jetstream().publish(event.subject, new TextEncoder().encode(JSON.stringify({ eventId: event.eventId, aggregateId: event.aggregateId, ...event.payload }))); } }
