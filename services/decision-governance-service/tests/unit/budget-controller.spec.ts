...
describe('ProposalController budget API', () => {
  it('创建并释放调仓批次预留', async () => {
    ...
    expect(controller.reserveBudget('proposal-api', input)).toMatchObject({ batchNumber: 1, status: 'RESERVED' });
    expect(controller.releaseBudget('r-api')).toMatchObject({ status: 'RELEASED' });
  });
});
