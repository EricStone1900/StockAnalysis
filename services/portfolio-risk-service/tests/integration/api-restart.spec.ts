...
suite('Portfolio API restart recovery', () => {
  ...
  it('continues versioned writes after recreating the Nest application', async () => {
    ...
    const first = await firstApp.getHttpAdapter().getInstance().inject({ method: 'POST', url: `/api/v1/portfolios/${portfolioId}/manual-snapshots`, headers: { 'Idempotency-Key': body.idempotencyKey }, payload: body });
    ...
    const second = await secondApp.getHttpAdapter().getInstance().inject({ method: 'POST', url: `/api/v1/portfolios/${portfolioId}/manual-snapshots`, headers: { 'Idempotency-Key': 'api-restart-key-2' }, payload: { ...body, expectedVersion: 1, idempotencyKey: 'api-restart-key-2' } });
