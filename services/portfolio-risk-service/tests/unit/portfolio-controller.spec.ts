...
describe('PortfolioController API mapping', () => {
  it('returns a snapshot and maps missing data to 404', async () => {
    ...
    await expect(controller.importOpening('p-1', 'key-1', 'actor-1', { idempotencyKey: 'key-1', actorId: 'actor-1' } as never)).resolves.toEqual(snapshot);
  ...
  it('maps version conflicts to 409 and validation errors to 400', async () => {
    ...
    const controller = new PortfolioController(service);
    await expect(controller.importOpening('p-1', 'key', 'actor', { idempotencyKey: 'key', actorId: 'actor' } as never)).rejects.toBeInstanceOf(ConflictException);
    await expect(controller.importOpening('p-1', 'key', 'actor', { idempotencyKey: 'other', actorId: 'actor' } as never)).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('exposes the reversal endpoint with idempotency validation', async () => {
    ...
    const controller = new PortfolioController(service);
    await expect(controller.reverse('p-1', 'entry-1', 'reverse-key', 'actor', { idempotencyKey: 'reverse-key', actorId: 'actor' } as never)).resolves.toEqual({ type: 'REVERSAL' });
    await expect(controller.reverse('p-1', 'entry-1', 'wrong', 'actor', { idempotencyKey: 'reverse-key', actorId: 'actor' } as never)).rejects.toBeInstanceOf(BadRequestException);
  });
});
