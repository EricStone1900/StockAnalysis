...
describe('ReconciliationController', () => {
  it('requires matching idempotency, actor and correlation headers', async () => {
    const service = { recordConfirmedFill: vi.fn().mockResolvedValue({ ledgerVersion: 2 }), recordCashDividend: vi.fn().mockResolvedValue({ ledgerVersion: 3 }), recordStockSplit: vi.fn().mockResolvedValue({ ledgerVersion: 4 }) };
    ...
    await expect(controller.applyStockSplit('split-1', 'actor-1', 'corr-1', { portfolioId: 'p-1', actorId: 'actor-1', idempotencyKey: 'split-1' } as never)).resolves.toEqual({ ledgerVersion: 4 });
  });
});
