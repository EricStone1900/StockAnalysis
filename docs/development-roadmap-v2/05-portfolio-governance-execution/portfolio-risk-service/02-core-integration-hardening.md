...
估值领域切片已支持以`marketDataVersion`、价格`asOf`和持仓快照版本计算持仓市值与总权益；任一持仓缺价、价格非正或超过最大允许陈旧时间均失败关闭。估值结果已落入不可变`portfolio_valuations`，以快照、行情版本和时间唯一约束防止重复。已在`packages/contracts/openapi/market-data.v1.yaml`冻结价格查询契约，market-data 实现和生成 Client 完成后再接入真实调用。
