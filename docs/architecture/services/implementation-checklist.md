...
- ADR-010：组合调仓批次、每日0～2批、预算预留及计数时点。
...
- ADR-019：免费优先准实时Watchlist的容量、新鲜度、失败关闭和生产行情升级门槛。
...
- 首版采用ADR-019的`FREE_TIERED_10_20_30`：每10分钟一次批量快照本地过滤，P0/P1/P2每10/20/30分钟评估并复用快照；默认50支、验收后最多80支、100支仅压力测试；免费源陈旧或故障必须失败关闭。
- 实现版本化MonitorPolicy和阈值动作映射；盘中层只延迟/取消、风险减仓或执行修正，不重新计算日频Alpha。
...
- 实现RiskEvaluation。
- 实现人工批准、拒绝、修改和过期。
- 实现组合级TradeProposal、DecisionBudgetReservation、RebalanceBatch和每日0～2批硬上限。
- 实现全局暂停和只观察模式。
...
21. 所有日频策略均可输出NO_REBALANCE；每日计算成功不增加组合调仓批次。
22. 一次多Leg调仓只占一个批次；并发预留、接受前释放和接受后不释放符合ADR-010。
