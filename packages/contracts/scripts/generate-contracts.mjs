...
const fields = Object.keys(schema.properties).map((name) => `  ${name}${required.has(name) ? '' : '?'}: ${schema.properties[name].type === 'integer' ? 'number' : name === 'payload' ? 'Record<string, unknown>' : 'string'};`).join('\n');
const ts = `// Generated from schemas/domain-event-envelope.schema.json. Do not edit.\nexport interface DomainEventEnvelope {\n${fields}\n}\n`;
const python = `# Generated from schemas/domain-event-envelope.schema.json. Do not edit.\nfrom typing import Any\nfrom pydantic import BaseModel\n\nclass DomainEventEnvelope(BaseModel):\n${Object.keys(schema.properties).map((name) => `    ${name}: ${schema.properties[name].type === 'integer' ? 'int' : name === 'payload' ? 'dict[str, Any]' : 'str'}${required.has(name) ? '' : ' | None = None'}`).join('\n')}\n`;
